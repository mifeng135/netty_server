...
public class CtrlAnnotation {
     ...
     */
    public void invokeMethod(int cmd, ChannelHandlerContext context, byte[] data) {
        ...
        try {
            methodAccess.invoke(oc, methodName, context, data);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public void invokeMethod(int cmd, byte[] data) {
        Method method = methodMap.get(cmd);
        if (method == null) {
            return;
        }
        ...
        String methodName = method.getName();
        try {
            methodAccess.invoke(oc, methodName, data);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
